...
})({"e8ea15e9561fa70f72670a60b263fb6b":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = 1234;
var HMR_ENV_HASH = "d751713988987e9331980363e24189ce";
module.bundle.HMR_BUNDLE_ID = "8fa1b09b8d4e2213c4f06f0c843c0bba";
...
},{}],"681b237cc4868bc01d1943a9fa22b7d7":[function(require,module,exports) {
/**
...
function KellyColorPicker(cfg) {
  ...
  var input = false; // used by updateInput() function if not overloaded by user event
  ...
  function initStyleSwitch() {
    ...
    styleSwitch.draw = function () {
      ...
      if (switchFig == 'quad') {
        ...
        var svmPos = {
          x: circleRadius * 2 + (circleRadiusMain - circleRadius),
          y: this.size / 2
        }; // start middle point
  ...
  function initColorSaver(align, selected, color) {
    ...
    colorSaver.updateSize = function () {
      this.width = parseInt(wheelBlockSize - wheelBlockSize / 100 * (100 - this.widthPercentage)); // start render point in global canvas coords

      if (this.align == 'left') {
        this.position = {
          x: 0,
          y: wheelBlockSize - this.width
        };
      } else if (this.align == 'right') {
        this.position = {
          x: wheelBlockSize - this.width,
          y: wheelBlockSize - this.width
        };
      }
    ...
    colorSaver.isDotIn = function (dot) {
      ...
      if (this.align == 'left') {
        ...
        path[1] = {
          x: path[0].x,
          y: path[0].y + this.width
        }; // bottom right

        path[2] = {
          x: path[0].x - this.width,
          y: this.position.y + this.width
        }; // bottom left
      }
  ...
  wheel.draw = function () {
    ...
    if (this.imageData) {
      ...
      for (var angle = 0; angle <= 360; angle++) {
        ...
        ctx.fillStyle = 'rgb(' + targetRgb.r + ', ' + targetRgb.g + ', ' + targetRgb.b + ')'; //ctx.fillStyle = 'hsl('+hAngle+', 100%, 50%)';
  ...
  alphaSlider.updateSize = function () {
    this.pos = {
      x: wheelBlockSize + alphaSlider.padding,
      y: alphaSlider.padding
    };
  ...
  svCursorMouse.initSvCursor = function () {
    ...
    if (!canvasHelper) return false; // create canvas on 2 pixels bigger for Opera that cut image
    ...
    if (this.cEl.style.cursor.indexOf(curImageData) === -1) {
  ...
  function constructor(cfg) {
    var criticalError = '',
        placeName = ''; // save non-camelased old style options compatibility
    ...
    if (cfg.input && typeof cfg.input !== 'object') {
      ...
      input = cfg.input; // if (!cfg.input) log += '| "input" (' + inputName + ') not not found';
    ...
    } else criticalError += '| "place" (' + placeName + ') not not found'; // attach directly to input by popup
    // hex default #000000
    ...
    if (cfg.resizeWith) {
      if (typeof cfg.resizeWith !== 'object' && typeof cfg.resizeWith !== 'boolean') cfg.resizeWith = document.getElementById(cfg.resizeWith);
      ...
      if (cfg.resizeSide) resizeSide = cfg.resizeSide;
  ...
  function getSizeByElement(el) {
    ...
    if (el === canvas) {
      if (sizeInfo.width <= sizeInfo.height) size = sizeInfo.height;else if (sizeInfo.height < sizeInfo.width) size = sizeInfo.width;
    } else {
      if (resizeSide) {
        if (resizeSide == 'height') size = sizeInfo.height;else if (resizeSide == 'width') size = sizeInfo.width;
      } else {
        if (sizeInfo.width > sizeInfo.height) size = sizeInfo.height;else if (sizeInfo.height >= sizeInfo.width) size = sizeInfo.width;
      }
    }
    ...
    if (typeof cString == 'string') {
      ...
      if (cString.indexOf('(') == -1) {
        ...
        if (cString.length >= 3) {
          ...
          h = cString; // complite full format, by replicating the R, G, and B values
          ...
          if (h.length == 8) alpha = (parseInt(h, 16) & 255) / 255;
        }
      } else {
        vals = cString.split(',');
      ...
      if (typeof cString.r != 'undefined' && typeof cString.g != 'undefined' && typeof cString.b != 'undefined') {
        // rgb input
        h = rgbToHex(cString);
      } else if (typeof cString.h != 'undefined' && typeof cString.s != 'undefined' && typeof cString.l != 'undefined') {
  ...
  function getSvFigureQuad() {
    ...
    quad.draw = function () {
      ...
      for (var i = 0; i <= this.path.length - 1; ++i) {
        if (i == 0) ctx.moveTo(this.path[i].x, this.path[i].y);else ctx.lineTo(this.path[i].x, this.path[i].y);
      }
    ...
    quad.updateSize = function () {
      var workD = wheel.innerRadius * 2 - wheelCursor.paddingX * 2 - this.padding * 2; // исходя из формулы диагонали квадрата, узнаем длинну стороны на основании доступного диаметра

      this.size = Math.floor(workD / Math.sqrt(2));
      this.path = new Array(); // находим верхнюю левую точку и от нее задаем остальные координаты
  ...
  function getSvFigureTriangle() {
    ...
    triangle.draw = function () {
      ...
      for (var i = 0; i <= trianglePath.length - 1; ++i) {
        if (i == 0) ctx.moveTo(trianglePath[i].x, trianglePath[i].y);else ctx.lineTo(trianglePath[i].x, trianglePath[i].y);
      }
    ...
    triangle.dotToSv = function (dot) {
      ...
      var len = getLen(p, this.vol[0]); // dirty tricks? replace output to interpolation and lerp in future
    ...
    triangle.updateSize = function () {
      ...
      this.outerRadius = wheel.innerRadius - wheelCursor.paddingX - this.padding; // из теоремы синусов треугольника
  ...
  function updateSize() {
    ...
    wheel.innerRadius = wheel.outerRadius - wheel.width; // объект относительно начала координат

    wheelCursor.path = [{
      x: wheel.innerRadius - wheelCursor.paddingX,
      y: wheelCursor.height * -1
    }, {
      x: wheel.outerRadius + wheelCursor.paddingX,
      y: wheelCursor.height * -1
    }, {
      x: wheel.outerRadius + wheelCursor.paddingX,
      y: wheelCursor.height
    }, {
      x: wheel.innerRadius - wheelCursor.paddingX,
      y: wheelCursor.height
    }, {
      x: wheel.innerRadius - wheelCursor.paddingX,
      y: (wheelCursor.height + wheelCursor.lineWeight / 2) * -1
    }];
  ...
  function updateInput(manualEnter) {
    ...
    if (!manualEnter) {
      switch (inputFormat) {
        case 'mixed':
          if (a < 1) input.value = rgba;else input.value = hex;
          break;
  ...
  function drawColorPicker() {
    if (!ctx) return false;
    ctx.clearRect(0, 0, canvas.width, canvas.height); // put buffered data
    ...
    drawColorSavers();
    if (styleSwitch) styleSwitch.draw(); // поместить текущее отрисованное изображение кольца + sv селектора в буфер
    // notice :
  ...
  function draw() {
    ...
    var curAngle = hsv.h * 360 - wheel.startAngle; // cursors
    ...
    ctx.beginPath();
    var wheelCursorPath = rotatePath2(wheelCursor.path, curAngle, {
      x: wheel.pos.x,
      y: wheel.pos.y
    });
    ...
    ctx.stroke();
    ctx.closePath(); // sv cursor

    if (hsv.v > 0.5 && hsv.s < 0.5) ctx.strokeStyle = 'rgba(0, 0, 0, 1)';else ctx.strokeStyle = 'rgba(255, 255, 255, 1)'; //ctx.strokeStyle='rgba(255,255, 255, 1)';
  ...
  this.setColorByHex = function (inputHex, manualEnter) {
    ...
    if (hex && inputHex == hex && rendered) return; // set and redraw all
  ...
  this.touchMoveEvent = function (e) {
    if (drag) {
      // todo check number of touches to ignore zoom action
  ...
  this.mouseDownEvent = function (event) {
    ...
    var move,
        up = false;
    var newDot = getEventDot(event); // console.log('mouseDownEvent : cur : ' + newDot.x + ' | ' + newDot.y);
  ...
  this.wheelMouseMove = function (event, dot) {
    ...
    cursorAnimReady = false;
    var newDot = getEventDot(event); // console.log('wheelMouseMove : start : ' + dot.x + ' | ' + dot.y + ' cur : ' + newDot.x + ' | ' + newDot.y);
  ...
  this.wheelMouseUp = function (event, dot) {
    event.preventDefault();
    if (!drag) return; //console.log('wheelMouseUp : start : ' + dot.x + ' | ' + dot.y);
  ...
  this.alphaMouseMove = function (event, dot) {
    ...
    cursorAnimReady = false;
    var newDot = getEventDot(event); // console.log('svMouseMove : start : ' + dot.x + ' | ' + dot.y + ' cur : ' + newDot.x + ' | ' + newDot.y);
  ...
  this.svMouseUp = function (event, dot) {
    event.preventDefault();
    if (!drag) return; // console.log('svMouseUp : start : ' + dot.x + ' | ' + dot.y);
    ...
    drag = false;
    var newDot = getEventDot(event);
    svCursorMouse.updateCursor(newDot); // todo
...
KellyColorPicker.attachToInputByClass = function (className, cfg) {
  ...
  for (var i = 0; i < inputs.length; i++) {
    if (cfg) cfg.input = inputs[i];else cfg = {
      input: inputs[i],
      size: 150
    };
...
},{}]},{},["e8ea15e9561fa70f72670a60b263fb6b"], null)

//# sourceMappingURL=lumenator.8fa1b09b.js.map
