...
function KellyColorPicker(cfg) {
	...
	function initStyleSwitch() {
		...
		styleSwitch.isDotIn = function(dot) {
			if (
				dot.x >= this.position.x &&
				dot.x <= this.position.x + this.size &&
				dot.y >= this.position.y &&
				dot.y <= this.position.y + this.size
			) {
				return true;
			}
	...
	function initColorSaver(align, selected, color) {
		...
		colorSaver.isDotIn = function(dot) {
			...
			if (Math.ceil(s) == Math.ceil(selfS)) return true;
			else return false;
		};
	...
	function getSizeByElement(el) {
		...
		if (el === canvas) {
			...
			else if (sizeInfo.height < sizeInfo.width) size = sizeInfo.width;
		} else {
			if (resizeSide) {
				if (resizeSide == 'height') size = sizeInfo.height;
				else if (resizeSide == 'width') size = sizeInfo.width;
			} else {
				if (sizeInfo.width > sizeInfo.height) size = sizeInfo.height;
				else if (sizeInfo.height >= sizeInfo.width) size = sizeInfo.width;
			}
	...
	function getP(point1, line1) {
		var l =
			(line1[0].x - line1[1].x) * (line1[0].x - line1[1].x) +
			(line1[0].y - line1[1].y) * (line1[0].y - line1[1].y);
		var pr =
			(point1.x - line1[0].x) * (line1[1].x - line1[0].x) + (point1.y - line1[0].y) * (line1[1].y - line1[0].y);
		var pt = true;
		var cf = pr / l;
	...
	function draw() {
		...
		if (alpha) {
			...
			ctx.rect(
				alphaSlider.pos.x - cursorPaddingX,
				alphaSlider.padding + pointY - cursorHeight / 2,
				alphaSlider.width + cursorPaddingX * 2,
				cursorHeight
			);
	...
	this.popUpShow = function(e) {
		...
		if (!KellyColorPicker.popupEventsInclude) {
			addEventListner(
				...
				'click',
				function(e) {
					if (KellyColorPicker.activePopUp) return KellyColorPicker.activePopUp.popUpClose(e);
					else return false;
				},
				'popup_close_'
			);
...
KellyColorPicker.attachToInputByClass = function(className, cfg) {
	...
	for (var i = 0; i < inputs.length; i++) {
		if (cfg) cfg.input = inputs[i];
		else cfg = { input: inputs[i], size: 150 };
...
export function KellyColorPicker();
